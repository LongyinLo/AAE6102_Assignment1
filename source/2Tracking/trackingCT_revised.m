...
%%Modification%%
Spacing = [-track.CorrelatorSpacing*1.5 -track.CorrelatorSpacing -track.CorrelatorSpacing*0.5 0 track.CorrelatorSpacing*0.5 track.CorrelatorSpacing track.CorrelatorSpacing*1.5];
%%End_Modification%%
...
%             rawsignal = rawsignal(1:2:length(rawsignal)) + 1i*rawsignal(2:2:length(rawsignal));% For NSL STEREO LBand only
        ...
        %%Modification%%
        t_Code_E75      = (0 + Spacing(1) + remChip) : codeFreq/signal.Fs : ((numSample -1) * (codeFreq/signal.Fs) + Spacing(1) + remChip);
        t_CodeEarly     = (0 + Spacing(2) + remChip) : codeFreq/signal.Fs : ((numSample -1) * (codeFreq/signal.Fs) + Spacing(2) + remChip);
        t_Code_E25      = (0 + Spacing(3) + remChip) : codeFreq/signal.Fs : ((numSample -1) * (codeFreq/signal.Fs) + Spacing(3) + remChip);
        t_CodePrompt    = (0 + Spacing(4) + remChip) : codeFreq/signal.Fs : ((numSample -1) * (codeFreq/signal.Fs) + Spacing(4) + remChip);
        t_Code_L25      = (0 + Spacing(5) + remChip) : codeFreq/signal.Fs : ((numSample -1) * (codeFreq/signal.Fs) + Spacing(5) + remChip);
        t_CodeLate      = (0 + Spacing(6) + remChip) : codeFreq/signal.Fs : ((numSample -1) * (codeFreq/signal.Fs) + Spacing(6) + remChip);
        t_Code_L75      = (0 + Spacing(7) + remChip) : codeFreq/signal.Fs : ((numSample -1) * (codeFreq/signal.Fs) + Spacing(7) + remChip);
        
        Code_E75     = Code(ceil(t_Code_E75) + 1);
        Code_E25     = Code(ceil(t_Code_E25) + 1);
        Code_L25     = Code(ceil(t_Code_L25) + 1);
        Code_L75     = Code(ceil(t_Code_L75) + 1);        
        %%End_Modification%%
        ...
        %%Adding%%
        E75_i  = sum(Code_E75   .*InphaseSignal);  E75_q = sum(Code_E75   .*QuadratureSignal);
        E25_i  = sum(Code_E25   .*InphaseSignal);  E25_q = sum(Code_E25   .*QuadratureSignal);
        L75_i  = sum(Code_L75   .*InphaseSignal);  L75_q = sum(Code_L75   .*QuadratureSignal);
        L25_i  = sum(Code_L25   .*InphaseSignal);  L25_q = sum(Code_L25   .*QuadratureSignal);
        %%End_Adding%%
        ...
        absL75_i = abs(L75_i);
        absL75_q = abs(L75_q);
        TckResultCT(Acquired.sv(svindex)).E75_i(Index) = absE75_i;
        TckResultCT(Acquired.sv(svindex)).E75_q(Index) = absE75_q;
        TckResultCT(Acquired.sv(svindex)).E_i(Index) = absE_i;
        TckResultCT(Acquired.sv(svindex)).E_q(Index) = absE_q;
        TckResultCT(Acquired.sv(svindex)).E25_i(Index) = absE25_i;
        TckResultCT(Acquired.sv(svindex)).E25_q(Index) = absE25_q;
        TckResultCT(Acquired.sv(svindex)).P_i(Index) = absP_i;
        TckResultCT(Acquired.sv(svindex)).P_q(Index) = absP_q;
        TckResultCT(Acquired.sv(svindex)).L25_i(Index) = absL25_i;
        TckResultCT(Acquired.sv(svindex)).L25_q(Index) = absL25_q;
        TckResultCT(Acquired.sv(svindex)).L_i(Index) = absL_i;
        TckResultCT(Acquired.sv(svindex)).L_q(Index) = absL_q;
        TckResultCT(Acquired.sv(svindex)).L75_i(Index) = absL75_i;
        TckResultCT(Acquired.sv(svindex)).L75_q(Index) = absL75_q;
        %%End_Modification%%        
